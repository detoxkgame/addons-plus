odoo.define('skit_credit_card.credit_card', function(require) {
	"use strict";		
...
models.Paymentline = models.Paymentline.extend({
    ...
    /** display extra charge amount for credit card **/
	get_extra_charge_amount:function(paymentline){
	    	var amt= paymentline.line_paid_amount-paymentline.amount
	    	return amt;
	   },
...
models.Order = models.Order.extend({
	...
	get_credit_card_charge : function(val) {
		...
		if (val != undefined) {
			if (val.journal.credit_card_charge > 0) {
				if (val.journal.is_credit_card_journal) {
			...
			for ( var i = 0; i < this.pos.cashregisters.length; i++ ) {
	               var  cashregister = this.pos.cashregisters[i];
	                if (cashregister.journal.credit_card_charge > 0){
	                	card_charge = cashregister.journal.credit_card_charge;
	                	return card_charge;
	                }
	...
	get_credit_card_value : function() {
		...
		for (var i = 0; i < lines.length; i++) {
			 ...
			 var credit_card_charge_added = cashregister.journal.credit_card_charge; 	
			 if(credit_card_charge_added>0){
	...
	add_paymentline : function(cashregister) {
		...
		if (cashregister.journal.type !== 'cash'
			...
			var selected_cashregister = cashregister;
            var card_charge = selected_cashregister.journal.credit_card_charge; 
			if(card_charge>0){
				var total = this.get_total_without_tax() + this.get_total_tax();
	    ...
	    payment_input: function(input) {
	        ...
	        if (newbuf !== this.inputbuffer) {
	            ...
	            if (order.selected_paymentline) {
	                ...
	                var selected_cashregister = order.selected_paymentline.cashregister;
		            var credit_card_charge_added = selected_cashregister.journal.credit_card_charge;	               
		            if(credit_card_charge_added>0)
		            {	var credit_per = selected_cashregister.journal.credit_card_charge;
	    ...
	    order_is_valid: function(force_validation) {
	        ...
	        if(is_order_valid){
		        ...
		        for (var i = 0; i < plines.length; i++) {
		            ...
		            var selected_cashregister = plines[i].cashregister;
		            var credit_card_charge_added = selected_cashregister.journal.credit_card_charge; 
		            if(credit_card_charge_added>0){	            		            	
		            	var credit_amount=plines[i].line_paid_amount;
		            	var amt=plines[i].amount;
		            	var diff = credit_amount-amt;
		            	if(diff!=0)
		            		plines[i].set_credit_card_charge_amount(diff);
		            	plines[i].set_amount(credit_amount); 
...
var CreditCardButton = screens.ActionButtonWidget.extend({
	...
	button_click : function() {
		...
		for (var i = 0; i < cash.length; i++) {
			var credit_card_charge_added = cash[i].journal.credit_card_charge;
			if (credit_card_charge_added>0) {
