odoo.define('skit_web_appointment.AppointmentController', function (require) {
"use strict";
...
var AppointmentController = AbstractController.extend({
    ...
    _onChangeDate: function (event) {
       ...
       // alert('modelData.scale'+modelData.scale)
...
//            }
//        } else 
        if (modelData.target_date.week() === event.data.date.week()) {
    ...
    _onOpenCreate: function (event) {
        ...
        var partner_ids = event.data.partner_ids.map(function(v) {
        	  return parseInt(v, 10);
        	});
        
        if (this.mapping.partner_ids) {
            context['default_' +this.mapping.partner_ids] = partner_ids || null;
        }
