odoo.define('skit_web_appointment.AppointmentModel', function (require) {
"use strict";
...
return AbstractModel.extend({
    ...
    calendarEventToRecord: function (event) {
        ...
        var partner_ids = [event.resourceId];
        var partner_id = partner_ids.map(function(v) {
        ...
        data[this.mapping.partner_ids] = [[6, 0, partner_id]];
    	...
    	var authorizedValues = [];
    	  _.each(this.data.filters, function (filter) {
              ...
              _.each(filter.filters, function (f) {
            	  ...
            	  if(filter.fieldName === 'partner_ids'){
            		  if (filter.write_model) {
            			  if (!authorizedValues[filter.fieldName])
            				  authorizedValues[filter.fieldName] = [];
            			  if (f.active) {
            				  authorizedValues[filter.fieldName].push(f.value);
            				  var headervalue ={id:f.value, title:f.label}
            				  partnername.push(headervalue);
            			  }
            		  }
            	  }
              });
    ...
    _getFullCalendarOptions: function () {
        return {
           // defaultView: "multiColAgendaDay",
        	defaultView: (this.mode === "month")? "month" : ((this.mode === "week")? "agendaWeek" : ((this.mode === "day")? "agendaDay" : "agendaDay")),
            header: false,
            ...
            views: {
                ...
                multiColAgendaDay: {
                    ...
                    duration: { days: 1 },
                    numColumns: 2,
                    columnHeaders: ['First column']
                }
            },
           
           

            resources: this._getPartnerNames(),
    ...
    _loadCalendar: function () {
        ...
        return $.when.apply($, defs).then(function () {
            ...
            .then(function (events) {
                ...
                self.data.fc_options.resources = self._getPartnerNames(); 
                return $.when(
