odoo.define('skit_website_fish_market.SaleOrder.instance', function (require) {
"use strict";
...
var OptionalShopProductsModal = require('skit_website_fish_market.OptionalProductsModalTest');

setInterval(function(){ 
	//var h = $('#sale_order_communication').find("h2").text();
	var url = window.location.href;
	var order = url.match(/my\/orders\/([0-9]+)/);
	if(order != null){
		var order_id = order[1];
		//alert("Hello"+order_id); 
		var url = "/sorder/" + parseInt(order_id);
        ajax.jsonRpc(url, 'call', {}).then(function (data) {
        	//$('#sale_order_communication').find("h2").text(data);
        	if(data == "sale"){
        		...
        		$('#sale_confirm_img').attr('src','/skit_website_fish_market/static/src/img/confirmed_green.png');
        	}
        		...
        		$('#sale_preparing_img').attr('src','/skit_website_fish_market/static/src/img/preparing_green.png');
        	}
        	else if(data == "ready"){
        		...
        		$('#sale_ready_img').attr('src','/skit_website_fish_market/static/src/img/ready_green.png');
        	}
        		...
        		$('#sale_delivered_img').attr('src','/skit_website_fish_market/static/src/img/delivered_green.png');
        	}
        });
	}
    ...
    _onShopAddCart: function (ev) {
    	...
    	this._rpc({
        	 ...
        	 if(data != undefined && data.indexOf('quantity') != -1){
        	 	...
        	 	//$('#float_new_web_cart_quantity').text(data);
        	 	//$('.my_cart_quantity').text(data);
        	/* self.rootProduct = {
                     product_id: 9,
        	 ...
        	 self.optionalProductsModalTest = new OptionalProductsModalTest($form, {
                 isWebsite: true,
