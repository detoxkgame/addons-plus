...
class Skit_saleorder(models.Model):
    ...
    def fetch_quotation_order(self, session_id):
        ...
        for order in orders:
            if (len(order.invoice_ids) > 1):
                for invoice in order.invoice_ids:
                    if not invoice.refund_invoice_id:
                        order_invoice_ids = invoice
            else:
                order_invoice_ids = order.invoice_ids
            if order_invoice_ids.state:
                if(order_invoice_ids.state == 'draft'):
                    status = 'Draft'
                elif(order_invoice_ids.state == 'open'):
                    status = 'Open'
                elif(order_invoice_ids.state == 'paid'):
                    status = 'Paid'
                elif(order_invoice_ids.state == 'cancel'):
                    status = 'Cancelled'
            else:
            ...
            isno = isno + 1
            unpaid_amount = round(order_invoice_ids.residual, 2)
            order_details.append({'name': order.name,
                                  'order_id': order.id,
                                  'sno': isno,
                                  ...
                                  'status': status,
                                  'invoice_id': order_invoice_ids.id,
