...
class PosConfig(models.Model):
    ...
    lock_return = fields.Boolean(string="Lock Product Return", default=False)
    return_password = fields.Char(string=u"Password")

    @api.constrains('return_password')
    def check_return_password(self):
        if self.lock_return is True:
            for item in str(self.return_password):
                try:
                    int(item)
