...
class PosOrder(models.Model):
    ...
    amount_round = fields.Float(string='Rounding Amount')

    @api.model
    def _order_fields(self, ui_order):
        order_fields = super(PosOrder, self)._order_fields(ui_order)
        order_fields['amount_round'] = ui_order.get('rounding', False)
        return order_fields
    ...
    def action_pos_order_invoice(self):
        ...
        for order in self:
            ...
            if order.amount_round != 0:
                rounding_product = config_id.rounding_product
                account = rounding_product.property_account_income_id or rounding_product.categ_id.property_account_income_categ_id
                new_invoice.update({
                    'invoice_line_ids': [(0, 0, {
                        'name': config_id.rounding_product.name,
                        'product_id': config_id.rounding_product.id,
                        'quantity': 1,
                        'price_unit': order.amount_round,
    ...
    def _create_account_move_line(self, session=None, move=None):
        ...
        for order in self.filtered(lambda o: not o.account_move or o.state == 'paid'):
            ...
            # Extra line for rounding
            config_id = order.config_id
            if config_id.enable_rounding and order.amount_round != 0:
                # Search for the income account
                if config_id.rounding_product.property_account_income_id.id:
                    income_account = config_id.rounding_product.property_account_income_id.id
                elif config_id.rounding_product.categ_id.property_account_income_categ_id.id:
                    income_account = config_id.rounding_product.categ_id.property_account_income_categ_id.id
                else:
                    raise UserError(_('Please define income '
                                      'account for this product: "%s" (id:%d).')
                                    % (config_id.rounding_product.name, config_id.rounding_product.id))

                name = config_id.rounding_product.name
                print(order.amount_round)
                data = {
                    'name': name,
                    'quantity': 1,
                    'product_id': config_id.rounding_product.id,
                    ...
                    'account_id': income_account,
                    'credit': ((order.amount_round > 0) and order.amount_round) or 0.0,
                    'debit': ((order.amount_round < 0) and -order.amount_round) or 0.0,
                    'tax_ids': [(6, 0, [])],
                    'partner_id': partner_id
                }
                print(data,"---------------------")
                if cur != cur_company:
                    data['currency_id'] = cur.id
                    data['amount_currency'] = -abs(order.amount_round) if data.get('credit') else abs(order.amount_round)
                    amount_cur_company += data['credit'] - data['debit']
                insert_data('product', data)
